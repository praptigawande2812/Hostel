...
function Suggestions() {
  ...
  const updateSuggestion = async (id) => {
    ...
    try {
      ...
      const data = await response.json();
      if (data.success) {
        toast.success("Suggestion Approved", { position: "top-right", autoClose: 3000 });
        getSuggestions();
      } else {
        toast.error("Something failed!", { position: "top-right", autoClose: 3000 });
      }
    } catch (error) {
      toast.error("Network error!", { position: "top-right", autoClose: 3000 });
    }
                ...
                >
                  <div className="flex items-center space-x-4">
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium truncate text-white">{suggestion.title}</p>
                      <p className="text-sm truncate text-gray-400">{suggestion.description}</p>
                      <button
